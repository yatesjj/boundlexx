...
def setup_test_container(dry_run=False):
    ...
    # Regex patterns
    port_pattern = re.compile(r'(["\']?)(\d{4,5})(["\']?):(\d{4,5})')  # e.g., "8001:8000"
    ...
    if not dry_run:
        print(f'All done! Test container is now unique to "{folder_name}" with port offsets.')
    else:
        print(f'Dry run complete. Would configure test container for "{folder_name}" with port offsets.')
